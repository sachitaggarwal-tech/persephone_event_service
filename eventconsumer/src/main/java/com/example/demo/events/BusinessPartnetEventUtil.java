...
import com.example.s4hana.S4HanaApiFacade;
import com.example.s4hana.proposal.Proposal;
import com.example.s4hana.proposal.ProposalAPIFacade;

public class BusinessPartnetEventUtil {

	private static Logger LOG = LoggerFactory.getLogger(BusinessPartnetEventUtil.class);
	...
	public static void handleBusinessPartnerEvent(String businessPartnerId) {
		if (businessPartnerId != null) {
			...
			Customer customer = new S4HanaApiFacade(accessor,apiHandler).getCustomerFromS4Hana(businessPartnerId);
			if (customer != null) {
	...
	public static void compareProposalToCustomer(Proposal proposal, Customer customer) {
		...
		if (proposal.equals(customer)) {
			...
			if ("new".equalsIgnoreCase(proposal.getState()) || "inProcess".equalsIgnoreCase(proposal.getState())) {
				new ProposalAPIFacade(accessor,apiHandler).rejectProposal("" + proposal.getProposalId(), customer.getId());
			}
		}
	}
