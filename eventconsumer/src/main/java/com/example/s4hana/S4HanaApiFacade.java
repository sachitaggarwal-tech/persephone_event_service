...
public class S4HanaApiFacade {
	...
	private final static String S4HANA_DEST = "S4HANA2";
	...
	public Customer getCustomerFromS4Hana(String customerId) {
		...
		try {
			...
			String response = apiHandler.executeGetCall(s4HanaUrl + "/API_BUSINESS_PARTNER/A_BusinessPartner('"+customerId+"')", headersMap);
			if(response != null) {
				...
				headersMap.put("Authorization", credentialToken);
				headersMap.put("Accept", "application/json");
				response = apiHandler.executeGetCall(s4HanaUrl + "/API_BUSINESS_PARTNER/A_BusinessPartner('"+customerId+"')/to_BusinessPartnerAddress", headersMap);
				if(response != null) {
					logger.debug(response);
